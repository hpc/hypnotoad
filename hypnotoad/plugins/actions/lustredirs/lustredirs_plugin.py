#
# An action plugin for hypnotoad to create lustre dirs for new users
#
...
class lustredirs_plugin(plugin.action_plugin):
    ...
    def setup(self, config, model_version):
        ...
        if config.getboolean('Action Options', 'lustredirs_plugin_enabled'):
            ...
            LOG.debug("Lustre Dirs plugin enabled")
    ...
    def teardown(self):
        ...
        if self.plugin_enabled:
            LOG.debug("Got to Lustre Dirs plugin teardown")

    def append_model(self, models):
        """Handle a model appended to this output."""
        if self.plugin_enabled:
            LOG.debug("Got to Lustre Dirs plugin append_model.")
